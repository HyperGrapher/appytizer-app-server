...
namespace appytizer {
SiteRegistry::SiteRegistry() { const auto path=ConfigStore::default_path().parent_path()/L"app.db"; std::error_code ec; std::filesystem::create_directories(path.parent_path(),ec); if (sqlite3_open_v2(path.string().c_str(),&db_,SQLITE_OPEN_READWRITE|SQLITE_OPEN_CREATE|SQLITE_OPEN_FULLMUTEX,nullptr)==SQLITE_OK) sqlite3_exec(db_,"CREATE TABLE IF NOT EXISTS sites(id INTEGER PRIMARY KEY,folder_name TEXT UNIQUE,path TEXT NOT NULL,detected_type TEXT NOT NULL,first_seen TEXT DEFAULT CURRENT_TIMESTAMP,last_seen TEXT DEFAULT CURRENT_TIMESTAMP);",nullptr,nullptr,nullptr); }
