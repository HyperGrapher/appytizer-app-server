...
std::vector<std::uint8_t> DnsServer::make_response(std::span<const std::uint8_t> query) {
  ...
  const auto type = read16(query, position); const bool ours = ends_with_ci(name, std::string(kSiteSuffix)); const std::size_t question_end = position + 4;
...
bool DnsServer::start() {
  ...
  socket_ = socket; running_ = true; thread_ = std::thread([this]{ run(); }); return true;
}
void DnsServer::stop() {
